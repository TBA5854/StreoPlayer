...
class SyncSocket(
        ...
        fun play(roomId: String, position: Double, startAt: Long = 0L) =
                send(
                        "sync:play",
                        JSONObject().apply {
                                put("user_id", userId)
        ...
        fun seek(roomId: String, position: Double, startAt: Long = 0L) =
                send(
                        "sync:seek",
                        JSONObject().apply {
                                put("user_id", userId)
        ...
        fun sendNtp() =
                send("sync:ntp", JSONObject().apply { put("t1", System.currentTimeMillis()) })
        ...
        private fun dispatch(event: String, payload: JSONObject) {
                ...
                when (event) {
                        ...
                        "queue:updated" ->
                                listener.onQueueUpdated(QueueUpdatedEvent.fromJson(payload))
